...
var serialNumber = pathname.substring(pathname.lastIndexOf('/') + 1);
...
function appendCourse(course, day, startTime, endTime, color) {
    ...
    var html = '<li class="single-event" data-start="' + startTime + '" data-end="' + endTime + '" ' + color + '>' +
        ...
        '<a href="#">' +
        '<em class="class-name">' + course['title'] + '</em>' +
        '<span class="professor-name">' + course['instructor'] + '</span>' +
        '<span class="class-room">' + course['location'] + '</span>' +
        '</a>' +
        '</li>';
...
$('#course-btn').click(function() {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            $.each(data['result'], function(i, course) {
